...
// 4. imagens
const WIDTHS = CFG.imageWidths || [240, 320, 400, 480, 600, 720, 960];
let imgCount = 0;
for (const el of $('img').toArray()) {
  ...
  try {
    ...
    if (uniq.length > 1) {
      ...
      // imagens lazy: o navegador usa a largura real renderizada (sizes=auto); as demais, estimativa
      const lazy = $img.attr('loading') === 'lazy';
      $img.attr('sizes', `${lazy ? 'auto, ' : ''}(max-width: 767px) 92vw, ${Math.min(dw, 800)}px`);
    }
    imgCount++;
